...
def test_portfolio_initializes_with_zero_values(portfolio):
...
def test_buy_operation_sets_average_price(portfolio):
    operation = create_stock_operation("buy", 10.0, 100)
    portfolio.register_operation(operation)
...
def test_multiple_buys_calculate_weighted_average(portfolio):
...
def test_sell_operation_decreases_quantity(portfolio):
...
def test_sell_with_profit_updates_total_profit(portfolio):
    buy = create_stock_operation("buy", 10.0, 100)
    sell = create_stock_operation("sell", 15.0, 50)
...
def test_sell_with_loss_updates_total_loss(portfolio):
...
def test_sequential_operations(portfolio):
